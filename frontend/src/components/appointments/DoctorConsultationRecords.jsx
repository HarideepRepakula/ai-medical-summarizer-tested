...
export default function DoctorConsultationRecords({ appointment, onBack }) {
	...
	useEffect(() => { loadRecords(); }, [appointment?.id]);
	...
	const sections = [
		{ id: 'summary',      label: 'Meeting Summary',  icon: '🧠' },
		...
		<div className="space-y-6 animate-fade-in">
			...
			<div className="card flex items-center justify-center py-20">
				<div className="flex flex-col items-center gap-3">
					<div className="w-10 h-10 border-2 border-gray-200 border-t-ai-500 rounded-full animate-spin" />
			...
			{activeSection === 'summary' && (
				<div className="card-ai animate-fade-in">
					...
					{records?.meetingSummary ? (
						<p className="text-sm text-text-primary leading-relaxed whitespace-pre-wrap">{records.meetingSummary}</p>
					) : (
					...
					<p className="text-[10px] text-text-secondary mt-4">
						⚕️ AI-generated from consultation audio. Verify with your own notes.
					</p>
				</div>
			)}
						...
						<div className="card">
							...
							<div className="bg-gray-50 rounded-clinical p-4 flex items-center justify-center min-h-[200px]">
								{records.prescriptionImageUrl.endsWith('.pdf') ? (
									<a href={records.prescriptionImageUrl} target="_blank" rel="noopener noreferrer" className="btn-primary btn-sm text-xs">
										📥 View / Download PDF
									</a>
								) : (
									<img src={records.prescriptionImageUrl} alt="Prescription" className="max-w-full max-h-[400px] rounded-clinical shadow-clinical" />
								)}
							</div>
						</div>
					)}
				...
				<div className="card animate-fade-in">
					<div className="flex items-center justify-between mb-4">
						...
						{records?.meetingTranscript && (
							<button onClick={() => setTranscriptExpanded(!transcriptExpanded)} className="btn-ghost btn-sm text-xs">
								{transcriptExpanded ? '🔽 Collapse' : '🔼 Expand Full'}
							</button>
						)}
					</div>
					{records?.meetingTranscript ? (
						<div className={`bg-gray-50 rounded-clinical p-4 overflow-hidden transition-all relative ${transcriptExpanded ? '' : 'max-h-[300px]'}`}>
						...
						<div className="text-center py-8 text-text-secondary text-sm">
							<span className="text-3xl block mb-2 opacity-60">🎤</span>
							Transcript will be available once AI processes the consultation audio.
						</div>
					)}
				</div>
			)}
							...
							{records.linkedRecords.map((file, i) => (
								<div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-clinical hover:bg-gray-100 transition-colors">
									...
									<div className="flex-1 min-w-0">
										...
										<p className="text-[11px] text-text-secondary">Uploaded {new Date(file.uploadedAt).toLocaleDateString()}</p>
									</div>
