						...
						{upcoming.map(apt => {
							...
							return (
								<tr key={apt.id} className="group">
									...
									<td>
										<LiveCountdown appointmentDate={apt.date} startTime={apt.startTime} />
									</td>
									<td>
									...
									<td>
										<div className="flex items-center gap-2">
											<button
												onClick={() => onConsult(apt)}
												className="btn-primary btn-sm text-xs whitespace-nowrap group-hover:shadow-clinical-md transition-shadow"
											>
												🩺 Consult
											</button>
											<div className="relative group/actions">
												...
												<div className="absolute right-0 top-full mt-1 bg-white border border-gray-100 rounded-clinical shadow-clinical-lg py-1 min-w-[140px] hidden group-hover/actions:block z-20">
													<button
														...
														className="w-full text-left px-3 py-2 text-xs text-text-secondary hover:bg-gray-50 hover:text-text-primary transition-colors"
													>
														📅 Reschedule
													</button>
													<button
														onClick={() => onCancel?.(apt)}
														className="w-full text-left px-3 py-2 text-xs text-danger-500 hover:bg-danger-50 transition-colors"
													>
														✕ Cancel
													</button>
												</div>
